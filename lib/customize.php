...
function infinity_customizer_register() {
	...
	$images = apply_filters( 'infinity_images', array( '1', '3', '5', '7', '8' ) );

	$wp_customize->add_section( 'infinity-settings', array(
		'description' => __( 'Use the included default images or personalize your site by uploading your own images.<br /><br />The default images are <strong>1600 pixels wide and 1000 pixels tall</strong>.', 'infinity' ),
		'title'    => __( 'Front Page Background Images', 'infinity' ),
		'priority' => 35,
	) );
	...
	foreach( $images as $image ) {
		...
		$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, $image .'-infinity-image', array(
			'label'    => sprintf( __( 'Featured Section %s Image:', 'infinity' ), $image ),
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'infinity_accent_color',
			array(
				'description' => __( 'Change the default color for some links, link hovers, buttons, and button hovers.', 'infinity' ),
			    'label'       => __( 'Accent Color', 'infinity' ),
			    'section'     => 'colors',
